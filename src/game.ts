...
export default class Demo extends Phaser.Scene {
    ...
    create() {
        ...
        this.scoreText = this.add
            .text(30, 20, `${this.score}`, { font: '45px Arial Black', color: '#ffe325' })
            .setStroke('#974c1e', 8);
        ...
        this.matter.add.gameObject(endLineSprite, {
            ...
            onCollideActiveCallback: (
                e: { bodyB: { velocity: { y: number } }; bodyA: { velocity: { y: number } } },
                body: any
            ) => {
                if (this.enableAdd) {
    ...
    onCompose(
        bodyA: {
            ...
            label: string;
            gameObject: { alpha: number };
            destroy: () => void;
        },
        bodyB: { gameObject: { alpha: number }; destroy: () => void }
    ) {
